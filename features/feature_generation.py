import json
import math, re, nltk, features.leetalpha as alpha, string, spacy, features.preprocessing as process, configparser
...
def vocabulary_size(string):
    ...
    for word in words:
        ...
        if not word in punctuation:
            voc_size.add(word)
    return (len(voc_size), len(voc_size) / count_words(string))
...
def feature_vector(string):
    ...
    string_remove_stop_words = remove_stop_words(string)
    string_lemmatize = lemmatize(string, language)
    string_remove_stop_words_lemmatize = lemmatize(string_remove_stop_words, language)
